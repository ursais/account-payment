    ...
    def execute_aging_query(self, age_date=False):
                ...
                CASE
                    WHEN (days_due BETWEEN 1 AND 30) THEN 
                        CASE
                            ...
                            AND aml.amount_residual < 0) THEN -(aml.credit - (SELECT coalesce(sum(apr.amount),0) FROM account_partial_reconcile apr
                            WHERE
                                (apr.credit_move_id = aml.id OR apr.debit_move_id = aml.id) AND apr.max_date <= '%s'))
                            WHEN (aml.full_reconcile_id is NULL and aml.amount_residual=0) THEN 0
                            WHEN (aml.full_reconcile_id IS NULL AND aml.amount_residual > 0) 
                                THEN aml.debit - (SELECT coalesce(sum(apr.amount),0) FROM account_partial_reconcile apr
        ...
        UNION
            SELECT
                ...
                0 AS not_due,
                CASE
                WHEN (aml.credit - (SELECT sum(l.debit) FROM account_move_line l WHERE l.full_reconcile_id = aml.full_reconcile_id 	AND l.date <='%s')) > 0 
                THEN -(aml.credit - (SELECT sum(l.debit) FROM 	account_move_line l WHERE l.full_reconcile_id = aml.full_reconcile_id AND l.date <= '%s'))
                ELSE 0 END AS total,
                NULL AS invoice_id,
