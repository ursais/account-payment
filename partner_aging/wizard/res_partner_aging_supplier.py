		...
		ai.invoice_date_due as inv_date_due
                ...
                CASE WHEN (days_due BETWEEN 1 and 30) THEN
                    CASE WHEN (aml.full_reconcile_id is NULL and
                        ...
                        WHEN (aml.full_reconcile_id is NULL and
                        aml.amount_residual=0) THEN
                            CASE WHEN aml.debit > 0 and aml.credit = 0 THEN 
                            -(aml.debit-(select
                            coalesce(sum(apr.amount),0) from account_partial_reconcile
                            apr where (apr.credit_move_id =aml.id or
                            apr.debit_move_id=aml.id) and apr.create_date <= '{}'))					
                            WHEN aml.credit > 0 and aml.debit = 0 THEN
                            aml.credit-(select
                            coalesce(sum(apr.amount),0) from account_partial_reconcile
                            apr where (apr.credit_move_id =aml.id or
                            apr.debit_move_id=aml.id) and apr.create_date <= '{}')
                        ...
                        apr where (apr.credit_move_id =aml.id or
                        apr.debit_move_id=aml.id) and apr.create_date <= '{}')
                        WHEN ai.move_type = 'in_refund' THEN
                        -(aml.debit-(select
                        coalesce(sum(apr.amount),0) from account_partial_reconcile
                        apr where (apr.credit_move_id =aml.id or
                        apr.debit_move_id=aml.id) and apr.create_date <= '{}'))
                ...
                LEFT JOIN 
                    account_account ac on ac.id = aml.account_id
                INNER JOIN
                (SELECT lt.id,
                    CASE WHEN inv.id is not null THEN
                        CASE WHEN inv.invoice_date_due is not NULL THEN '{}' -  inv.invoice_date_due
                        ELSE '{}' - lt.date 
                        END
                    ELSE '{}' - lt.date
                    END AS days_due
                    FROM account_move_line lt LEFT JOIN account_move inv
                    on lt.move_id = inv.id
                ) DaysDue ON DaysDue.id = aml.id
                LEFT JOIN 
                    account_move as ai ON ai.id = aml.move_id
                WHERE 
                    ac.user_type_id in (select id from account_account_type where type = 'payable') 
                    AND aml.date <= '{}' 
                    AND ai.state = 'posted' 
